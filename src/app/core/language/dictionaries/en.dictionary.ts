import { LanguageDictionary } from '../language.types';
// TODO(@Janberk): Import en-US.json from a real json file and export securely here.
export const EN_DICTIONARY: LanguageDictionary = {
  'language.en': 'English',
  'language.tr': 'Turkish',
  'auth.title': 'Login',
  'auth.hint': 'Use any email/password. Example: hr@dotqit.dev / 123456',
  'auth.email': 'Email',
  'auth.password': 'Password',
  'auth.login': 'Login',
  'auth.resendActivation': 'Resend activation code',
  'auth.loggingIn': 'Logging in...',
  'auth.error.required': 'Email and password are required.',
  'outage.status.0': 'Detected',
  'outage.status.1': 'Confirmed',
  'outage.status.2': 'Dispatched',
  'outage.status.3': 'Restoring',
  'outage.status.4': 'Restored',
  'outage.status.5': 'Cancelled',
  'outage.status.unknown': 'Unknown',
};
